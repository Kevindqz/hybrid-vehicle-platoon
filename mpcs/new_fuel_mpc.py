...
class FuelMpcCentNew(PwaMpc):
    ...
    coefficients_b = Params.coefficients_b
    coefficients_c = Params.coefficients_c

    def __init__(
        self,
        N: int,
        system: dict,
        fuel_penalize: float = 1.0,
    ) -> None:
        ...
        self.ep, _, _ = self.variable("ep", (1, self.N), lb = 0, ub = 1, discrete=True)
        # self.acc, _ = self.state("acc", 1, self.N)
        # self.f, _ = self.state("f", 1, self.N)

        self.set_pwa_dynamics(system)
        self.setup_cost_and_constraints(self.fuel_penalize)
    ...
    def setup_cost_and_constraints(
        ...
        # fuel consumption cost，which is polynomial
        cost += sum(
            [
                self.ep[0, k] * fuel_penalize * (self.coefficients_b[0] +
                self.coefficients_b[1] * self.x[1, k] +
                self.coefficients_b[2] * self.x[1, k] ** 2 +
                self.coefficients_b[3] * self.x[1, k] ** 3 +
                (self.x[1, k+1]- self.x[1, k]) / self.ts * (
                    self.coefficients_c[0] +
                    self.coefficients_c[1] * self.x[1, k] +
                    self.coefficients_c[2] * self.x[1, k] ** 2
                ))  
                for k in range(self.N)
            ]
        )

        # when u is negative, the fuel consumption is zero
        self.constraint("fuel1", self.u[0, :], ">=", self.ep[0, :] - 1)
        self.constraint("fuel2", self.u[0, :], "<=", self.ep[0, :])
    ...
    # def set_fuel_penalize(self, fuel_penalize: float) -> None:
    #     self.fuel_penalize = fuel_penalize